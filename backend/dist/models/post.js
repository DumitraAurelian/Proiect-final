...
const findOne = (postId, callback) => {
    const queryString = `SELECT p.id, p.titlu, p.continut, 
  p.poza, p.user_id, p.categorie_id, p.dataadaugare, c.nume 
  FROM posts AS p INNER JOIN categories AS c ON p.categorie_id = c.id WHERE p.id=?`;
